...
def get_time(image_path):
    with open(image_path, 'rb') as f:
        img = Image(f)
        return datetime.strptime(img.get("datetime_original"), '%Y:%m:%d %H:%M:%S')
...
def calculate_top_percent_distance(coords1, coords2, top_percent=TOP_PERCENTILE):
    ...
    top_distances = distances[:top_count]
    return np.mean(top_distances)
...
for i in range(len(captured_images)):
    for j in range(i + 1, min(i + 1 + COMPARE_LIMIT, len(captured_images))):
        ...
        if dt < MIN_TIME_DIFF or dt > MAX_TIME_DIFF:
            continue
        img1_cv, img2_cv = convert_to_cv(img1), convert_to_cv(img2)
...
with open(result_file, "w") as f:
    f.write(f"{mean_speed:.5f}")

print(f"Estimated ISS speed: {mean_speed:.5f} km/s")
